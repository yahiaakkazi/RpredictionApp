...
Fenetre_Simple=function() {
  ...
  Calcul_CFE_SIMPLEE=function() {
    r=Calcul_MAD_SIMPLE(Lissage_Simple(as.double(tclvalue(DebutVariable)),as.double(tclvalue(AlphaVariable))),as.double(tclvalue(DebutVariable)))
    g=Calcul_CFE_SIMPLE(Lissage_Simple(as.double(tclvalue(DebutVariable)),as.double(tclvalue(AlphaVariable))),as.double(tclvalue(DebutVariable)))
    h=Calcul_MSE_SIMPLE(Lissage_Simple(as.double(tclvalue(DebutVariable)),as.double(tclvalue(AlphaVariable))),as.double(tclvalue(DebutVariable)))
...
Lissage_Triple=function(Debut,alpha,beta,gamme) {
  ...
  y=Calcul_Ptcorrigé_Ecart_EcartAbsolu(Calcul_St_Lt_Tt_Pt(Calcul_St_Lt_Tt_Pt_Init(x,Debut),Debut,alpha,beta,gamme),Debut)
...
Fenetre=function() {
  ...
  Lissage_Triplee=function() {
    ...
    tkmessageBox(message=paste("Merci de trouver le resultat dans le nouveau excel crée dans le bureau sous le nom de test6.xlsx, veuillez aussi créer une copie car le document va se faire écraser dans le cas d'un nouveau usage du script"))
    return(p)
  }
...
Fenetre_Principale=function() {
  tt <- tktoplevel()
  tkwm.title(tt,"Application GOP")
  Triplee<- function() {
    Fenetre()
    tkmessageBox(message=paste("fichier chargÃ© avec succÃ¨s"))
  }
